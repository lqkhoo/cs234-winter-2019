...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        height, width, nchannels = state_shape # Input parameters. N-channel RGB or BW image
        self.s          = tf.placeholder(name='state', shape=[None, height, width, nchannels*self.config.state_history], dtype=tf.uint8)
        self.a          = tf.placeholder(name='action', shape=[None], dtype=tf.int32)
        self.r          = tf.placeholder(name='reward', shape=[None], dtype=tf.float32)
        self.sp         = tf.placeholder(name='nextState', shape=[None, height, width, nchannels*self.config.state_history], dtype=tf.uint8)
        self.done_mask  = tf.placeholder(name='doneMask', shape=[None], dtype=tf.bool)
        self.lr         = tf.placeholder(name='learnRate', shape=[], dtype=tf.float32)
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope):
            ...
            x = tf.layers.dense(x, units=num_actions, activation=None, reuse=reuse)
            out = x
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        # Grab the weights. These are lists of tf.Variables
        ...
        # For each instance tf.Variable, assign to its counterpart in the target network
        op = [tf.assign(target_w[i], w[i]) for i in range(len(w))]
    ...
    def add_loss_op(self, q, target_q):
        ...
        q_estimate = tf.reduce_sum(tf.one_hot(self.a, num_actions)*q, axis=1)
        self.loss = tf.reduce_mean(tf.squared_difference(q_samp, q_estimate))
    ...
    def add_optimizer_op(self, scope):
        ...
        w = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope)
        dL_dw = optimizer.compute_gradients(self.loss, w) # list of (grad, var the grad is wrt to)
        if self.config.grad_clip:
            for grad, _ in dL_dw:
                grad = tf.clip_by_norm(grad, self.config.clip_val)
        self.train_op = optimizer.apply_gradients(dL_dw)
        grad, _ = zip(*dL_dw)
