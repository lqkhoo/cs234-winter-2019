...
parser.add_argument('--env_name', required=True, type=str,
                    choices=['cartpole', 'pendulum', 'cheetah', 'antx', 'antxneg', 'anty', 'antyneg'])
...
def build_mlp(mlp_input, output_size, scope, n_layers, size, output_activation=None):
  with tf.variable_scope(scope):
    ...
    for i in range(n_layers): # hidden layers
      x = tf.layers.dense(x, size, activation=tf.nn.relu, name="hdense"+str(i)) # Size is a scalar; size of each hidden layer is the same
    x = tf.layers.dense(x, output_size, activation=output_activation, name="outdense")
    return x
...
class PG(object):
  ...
  def add_placeholders_op(self):
    ...
    self.advantage_placeholder = tf.placeholder(tf.float32, shape=(None,)) # These are just scalars


  def build_policy_network_op(self):
    scope = self.config.env_name + "_policy_network"
    if self.discrete:
  ...
  def add_loss_op(self):
    ...
    self.loss = tf.reduce_mean(self.loss) # average across minibatch
  ...
  def add_baseline_op(self):
    scope = self.config.env_name + "_baseline"
    baseline = build_mlp(
  ...
  def build(self):
    ...
    self.add_placeholders_op() # add placeholders
    self.build_policy_network_op() # create policy net
    self.add_loss_op() # add square loss
    self.add_optimizer_op() # add optmizer for the main networks
    if self.config.use_baseline: # add baseline
  ...
  def initialize(self):
    ...
    """
    self.sess = tf.Session() # create tf session
    self.add_summary() # tensorboard stuff
    init = tf.global_variables_initializer() # initiliaze all variables
    self.sess.run(init)
  ...
  def train(self):
    ...
    """
    print("############")
    print("obs_dim: " + str(self.observation_dim))
    print("action_dim: " + str(self.action_dim))
    ...
    for t in range(self.config.num_batches):
      ...
      saver.save(self.sess, self.config.model_output)

    print("ffffffffffffffffff")
    foo = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=self.config.env_name + '_policy_network')
    pprint(foo)
...
if __name__ == '__main__':
    ...
    if args.env_name == 'antx':
      env = AntEnvX()
    elif args.env_name == 'antxneg':
      env = AntEnvXNeg()
    elif args.env_name == 'anty':
      env = AntEnvY()
    elif args.env_name == 'antyneg':
      env = AntEnvYNeg()
    else:
